...
<div id="<?php echo $id ?>-container" <?php if ( isset( $deps ) ): ?>data-field="<?php echo $id ?>" data-dep="<?php echo $deps['ids'] ?>" data-value="<?php echo $deps['values'] ?>" <?php endif ?>>
    ...
    <p>
        <?php foreach ( $fields as $field_name => $field_label ) : ?>
            ...
            <input type="text" name="<?php echo $name ?>[<?php echo $field_name ?>]" id="<?php echo $id ?>_<?php echo $field_name ?>" value="<?php echo isset( $value[$field_name] ) ? str_replace( '"', "'", $value[$field_name] ) : '' ?>"<?php echo $size ?> /> &nbsp; &nbsp;
        <?php endforeach ?>
    </p>
</div>