...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
        ...
        <tr>
            ...
            <th class="product-name"><span class="nobr"><?php _e( 'Название', 'yit' ) ?></span></th>
            <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th class="product-price"><span class="nobr"><?php _e( 'Цена', 'yit' ) ?></span></th><?php endif ?>
            <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th><span class="nobr"><?php _e( 'Наличие', 'yit' ) ?></span></th><?php endif ?>
        ...
        if( count( $wishlist ) > 0 ) :
            foreach( $wishlist as $values ) :
                ...
                if( $product_obj !== false && $product_obj->exists() ) : ?>
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        ...
                        <td class="product-remove"><div><a href="javascript:void(0)" onclick="<?php echo $remove_wishlist ?>" class="remove" title="<?php _e( 'Удалить этот товар', 'yit' ) ?>">&times;</a></td>
                        ...
                        <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
                            <td class="product-price">
                                ...
                                if( $product_obj->price != '0' ) {
                                    ...
                                    echo apply_filters( 'yith_free_text', __( '', 'yit' ) );
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
                            <td class="product-stock-status">
                                ...
                                if( $stock_status == 'out-of-stock' ) {
                                    $stock_status = "Out";
                                    echo '<span class="wishlist-out-of-stock">' . __( 'Нет в наличии', 'yit' ) . '</span>';
                                } else {
                                    $stock_status = "In";
                                    echo '<span class="wishlist-in-stock">' . __( 'В наличии', 'yit' ) . '</span>';
                                }
                                ?>
                            </td>
                        <?php endif ?>
            ...
            <tr>
                <td colspan="6" class="wishlist-empty"><?php _e( 'Вы еще не добавили товары в свой список желаний', 'yit' ) ?></td>
            </tr>
        <?php
        endif;
