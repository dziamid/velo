...
if( $_GET['action'] == 'remove_from_wishlist' ) {
    ...
    if( $yith_wcwl->remove( $_GET['wishlist_item_id'] ) )
        { _e( 'Товар успешно удален.', 'yit' ); }
    else {
        echo '#' . $count . '#';
        _e( 'Ошибка. Не удается удалить товар из списка желаний.', 'yit' );
    }
...
elseif( $_GET['action'] == 'add_to_wishlist' ) {
    ...
    if( $return == 'true'  )
        { echo $return . '##' . __( 'Товар добален!', 'yit' ); }
    elseif( $return == 'exists' )
        { echo $return . '##' . __( 'Товар уже добавлен в список ваших желаний.', 'yit' ); }
