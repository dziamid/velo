...
if( !class_exists( 'YITH_WCWL' ) ) {    
    ...
    class YITH_WCWL {
        ...
        public function add() {
            ...
            if ( isset( $this->details['add_to_wishlist'] ) && is_numeric( $this->details['add_to_wishlist'] ) ) {
                ...
                if( $return ) {
                    ...
                    $this->errors[] = __( 'Произошла ошибка при добавлении товара в список ваших желаний.', 'yit' );
                    return "error";
                }
            } 
        }
        ...
        public function remove( $id ) {
            ...
            if( is_user_logged_in() ) {
                ...
                if( $wpdb->query( $sql ) ) {
                    ...
                    $this->errors[] = __( 'Произошла ошибка во время удаления товара из списка ваших желаний', 'yit' );
                    return false;
                }
            } elseif( yith_usecookies() ) {
        ...
        public function add_to_wishlist_ajax() {    
            ...
            if( $return == 'true'  )
                { echo $return . '##' . __( 'Товар добавлен!', 'yit' ); }
            elseif( $return == 'exists' )
                { echo $return . '##' . __( 'Товар уже находится в списке ваших желаний.', 'yit' ); }
        ...
        public function remove_from_wishlist_ajax() {   
            ...
            if( $this->remove( $_GET['wishlist_item_id'] ) )
                { echo apply_filters( 'yith_wcwl_product_removed_text', __( 'Товар успешно удален.', 'yit' ) ); }
            else {
                echo '#' . $count . '#';
                _e( 'Ошибка. Не удалось удалить товар.', 'yit' );
            }
            
            if( !$count )
                { _e( 'Вы еще не добавили товары в список ваших желаний', 'yit' ); }
