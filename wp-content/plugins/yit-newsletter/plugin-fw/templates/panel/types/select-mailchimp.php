        ...
        <div class="select_wrapper">
            <select name="<?php echo $this->get_name_field( $option['id'] ) ?>" id="<?php echo $this->get_id_field( $option['id'] ) ?>">
                <?php foreach( $option['options'] as $key => $value ) : ?>
                    <option value="<?php echo $key ?>"<?php selected( $key, $db_value ) ?>><?php echo $value ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <input type="button" class="button-secondary <?php echo $option['class']?>" value="<?php echo $option['button_name']?>"/>
