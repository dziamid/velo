            ...
            if( isset( $list ) && strcmp( $list, '-1' ) != 0 && isset( $apikey ) && strcmp( $apikey, '' ) != 0 && check_ajax_referer( 'yit_mailchimp_newsletter_form_nonce', 'yit_mailchimp_newsletter_form_nonce', false ) ){
                ...
                if($result['error_count'] != 0){
                    ...
                    foreach($result['errors'] as $error){
                        ...
                        if( $code <= 0 ){
                            $message .= __( 'Mailchimp server error', 'yit' );
                        }
                        elseif($code >= 100 && $code < 120 ){
                            $message .= __( 'Mailchimp user related error', 'yit' );
                        }
                        elseif($code >= 120 && $code < 200 ){
                            $message .= __( 'Mailchimp user related error (action)', 'yit' );
                        }
                        elseif($code >= 200 && $code < 210 ){
                            $message .= __( 'Mailchimp list related error', 'yit' );
                        }
                        elseif($code >= 210 && $code < 220 ){
                            $message .= __( 'Mailchimp list related error (basic action)', 'yit' );
                        }
                        elseif($code >= 220 && $code < 230 ){
                            $message .= __( 'Mailchimp list related error (import)', 'yit' );
                        }
                        elseif($code >= 230 && $code < 250 ){
                            $message .= __( 'Mailchimp list related error (email)', 'yit' );
                        }
                        elseif($code >= 250 && $code < 270 ){
                            $message .= __( 'Mailchimp list related error (merge)', 'yit' );
                        }
                        elseif($code >= 270 && $code < 300 ){
                            $message .= __( 'Mailchimp list related error (interest group)', 'yit' );
                        }
                        else{
                            $message .= __( 'Mailchimp general error', 'yit' );
                        }
