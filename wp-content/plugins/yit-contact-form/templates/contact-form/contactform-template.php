...
<form id="contact-form-<?php echo $form_name ?>" class="contact-form<?php ( !$is_footer ? ' row-fluid' : '' )?>" method="post" action="" enctype="multipart/form-data">
    ...
    <fieldset>
        <ul>
            ...
            foreach ( $fields as $id => $field ) {
                ...
                <li class="<?php echo '' . implode( $li_class, ' ' ) . ' ' . $field['width'] ?>">
                    ...
                    <div class="input-prepend">
                        ...
                        yit_plugin_get_template(untrailingslashit( plugin_dir_path( __FILE__ ) ), '../fields/'.$field['type'].'.php', array('field'=>$field, 'form_name'=>$form_name, 'value'=>$value, 'input_class'=>$input_class, 'title_position' => $title_position ) );
                        ?>
            ...
            <li class="submit-button col-sm-<?php echo $max_width?> no-padding-right">
                ...
                <input type="submit" name="yit_sendemail" value="<?php echo YIT_Contact_Form()->get('_submit_label') ?>" class="<?php echo $button_style . ' ' .  YIT_Contact_Form()->get('_submit_alignment'); ?>" />
