...
function hybrid_loginout_link_shortcode() {
	if ( is_user_logged_in() )
		$out = '<a class="logout-link" href="' . esc_url( wp_logout_url( site_url( $_SERVER['REQUEST_URI'] ) ) ) . '" title="' . esc_attr__( 'Log out', 'hybrid-core' ) . '">' . __( 'Выход', 'hybrid-core' ) . '</a>';
	else
		$out = '<a class="login-link" href="' . esc_url( wp_login_url( site_url( $_SERVER['REQUEST_URI'] ) ) ) . '" title="' . esc_attr__( 'Log in', 'hybrid-core' ) . '">' . __( 'Вход', 'hybrid-core' ) . '</a>';
...
function hybrid_query_counter_shortcode() {
	if ( current_user_can( 'edit_theme_options' ) )
		return sprintf( __( 'Эта страница загружалась в течение% 1$s секунд% 2$s запросов к базе данных', 'hybrid-core' ), timer_stop( 0, 3 ), get_num_queries() );
	return '';
}
...
function hybrid_entry_edit_link_shortcode( $attr ) {
	...
	return $attr['before'] . '<span class="edit"><a class="post-edit-link" href="' . esc_url( get_edit_post_link( get_the_ID() ) ) . '" title="' . sprintf( esc_attr__( 'Редактировать %1$s', 'hybrid-core' ), $post_type->labels->singular_name ) . '">' . __( 'Редактировать', 'hybrid-core' ) . '</a></span>' . $attr['after'];
}
...
function hybrid_entry_comments_link_shortcode( $attr ) {
	...
	$attr = shortcode_atts( array( 'zero' => __( 'Оставьте ответ', 'hybrid-core' ), 'one' => __( '%1$s Ответ', 'hybrid-core' ), 'more' => __( '%1$s Ответы', 'hybrid-core' ), 'css_class' => 'comments-link', 'none' => '', 'before' => '', 'after' => '' ), $attr );
	...
	elseif ( 0 == $number )
		$comments_link = '<a class="' . esc_attr( $attr['css_class'] ) . '" href="' . get_permalink() . '#respond" title="' . sprintf( esc_attr__( 'Комментарий к %1$s', 'hybrid-core' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $attr['zero'], number_format_i18n( $number ) ) . '</a>';
	elseif ( 1 == $number )
		$comments_link = '<a class="' . esc_attr( $attr['css_class'] ) . '" href="' . get_comments_link() . '" title="' . sprintf( esc_attr__( 'Комментарий к %1$s', 'hybrid-core' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $attr['one'], number_format_i18n( $number ) ) . '</a>';
	elseif ( 1 < $number )
		$comments_link = '<a class="' . esc_attr( $attr['css_class'] ) . '" href="' . get_comments_link() . '" title="' . sprintf( esc_attr__( 'Комментарий к %1$s', 'hybrid-core' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $attr['more'], number_format_i18n( $number ) ) . '</a>';
...
function hybrid_entry_title_shortcode( $attr ) {
	...
	if ( empty( $title ) && !is_singular() )
		$title = "<{$tag} class='{$class}'><a href='" . get_permalink() . "'>" . __( '(Без названия)', 'hybrid-core' ) . "</a></{$tag}>";
...
function hybrid_entry_permalink_shortcode( $attr ) {
	...
	return $attr['before'] . '<a href="' . esc_url( get_permalink() ) . '" class="permalink">' . __( 'Ссылка', 'hybrid-core' ) . '</a>' . $attr['after'];
}
...
function hybrid_comment_permalink_shortcode( $attr ) {
	...
	$attr = shortcode_atts( array( 'before' => '', 'after' => '' ), $attr );
	$link = '<a class="permalink" href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '" title="' . sprintf( esc_attr__( 'Ссылка на комментарий %1$s', 'hybrid-core' ), $comment->comment_ID ) . '">' . __( 'Ссылка', 'hybrid-core' ) . '</a>';
...
function hybrid_comment_edit_link_shortcode( $attr ) {
	...
	$link = '<a class="comment-edit-link" href="' . esc_url( $edit_link ) . '" title="' . sprintf( esc_attr__( 'Редактировать %1$s', 'hybrid-core' ), $comment->comment_type ) . '"><span class="edit">' . __( 'Редактировать', 'hybrid-core' ) . '</span></a>';
...
function hybrid_comment_reply_link_shortcode( $attr ) {
	...
	$defaults = array(
		'reply_text' => __( 'Ответ', 'hybrid-core' ),
		'login_text' => __( 'Войдите чтобы ответить.', 'hybrid-core' ),
