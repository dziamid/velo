...
function hybrid_comment_form_args( $args ) {
	...
	$fields = array(
		'author' => '<p class="form-author' . esc_attr( $input_class ) . '"><label for="author">' . __( 'Имя', 'hybrid-core' ) . $req . '</label> <input type="text" class="text-input" name="author" id="author" value="' . esc_attr( $commenter['comment_author'] ) . '" size="40" /></p>',
		...
		'url' => '<br><p class="form-url"><label for="url">' . __( 'Сайт', 'hybrid-core' ) . '</label><input type="text" class="text-input" name="url" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="20" /></p>'
	);

	/* Sets the default arguments for displaying the comment form. */
	$args = array(
		'fields' => apply_filters( 'comment_form_default_fields', $fields ),
		'comment_field' => '<p class="form-textarea req"><label for="comment">' . __( 'Текст:', 'hybrid-core' ) . '</label><textarea name="comment" id="comment" cols="60" rows="10"></textarea></p>',
		'must_log_in' => '<p class="alert">' . sprintf( __( 'Вы должны <a href="%1$s" title="Log in">войти под своим логином</a> чтобы оставить комментарий', 'hybrid-core' ), wp_login_url( get_permalink() ) ) . '</p><!-- .alert -->',
		'logged_in_as' => '<p class="log-in-out">' . sprintf( __( 'Вы вошли как: <a href="%1$s" title="%2$s">%2$s</a>.', 'hybrid-core' ), admin_url( 'profile.php' ), esc_attr( $user_identity ) ) . ' <a href="' . wp_logout_url( get_permalink() ) . '" title="' . esc_attr__( 'Выйти из этого аккаунта', 'hybrid-core' ) . '">' . __( 'Выход &raquo;', 'hybrid-core' ) . '</a></p><!-- .log-in-out -->',
		'comment_notes_before' => '',
		'comment_notes_after' => '',
		'id_form' => 'commentform',
		'id_submit' => 'submit',
		'title_reply' => __( 'Оставить комментарий', 'hybrid-core' ),
		'title_reply_to' => __( 'Оставить комментарий к %s', 'hybrid-core' ),
		'cancel_reply_link' => __( 'Удалить комментарий.', 'hybrid-core' ),
		'label_submit' => __( 'Отправить', 'hybrid-core' ),
	);
