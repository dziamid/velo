	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date () ) {
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Архив за %1$s', 'hybrid-core' ), get_the_time( __( 'g:i a', 'hybrid-core' ) ) );

			elseif ( get_query_var( 'minute' ) )
				$doctitle = sprintf( __( 'Архив за минуту %1$s', 'hybrid-core' ), get_the_time( __( 'i', 'hybrid-core' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Архив за %1$s', 'hybrid-core' ), get_the_time( __( 'g a', 'hybrid-core' ) ) );

			elseif ( is_day() )
				$doctitle = sprintf( __( 'Архив за %1$s', 'hybrid-core' ), get_the_time( __( 'F jS, Y', 'hybrid-core' ) ) );

			elseif ( get_query_var( 'w' ) )
				$doctitle = sprintf( __( 'Архив за неделю %1$s of %2$s', 'hybrid-core' ), get_the_time( __( 'W', 'hybrid-core' ) ), get_the_time( __( 'Y', 'hybrid-core' ) ) );

			elseif ( is_month() )
				$doctitle = sprintf( __( 'Archive for %1$s', 'hybrid-core' ), single_month_title( ' ', false) );

			elseif ( is_year() )
				$doctitle = sprintf( __( 'Архив за %1$s', 'hybrid-core' ), get_the_time( __( 'Y', 'hybrid-core' ) ) );
		}

		/* For any other archives. */
		else {
			$doctitle = __( 'Архивы', 'hybrid-core' );
		}
	}

	/* If viewing a search results page. */
	elseif ( is_search() )
		$doctitle = sprintf( __( 'Результат поиска &quot;%1$s&quot;', 'hybrid-core' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 not found page. */
	elseif ( is_404() )
		$doctitle = __( '404 Не найдена', 'hybrid-core' );
