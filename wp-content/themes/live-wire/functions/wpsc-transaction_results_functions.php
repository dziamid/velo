...
function wpsc_transaction_theme() {
	...
	if(!$dont_show_transaction_results ) {
		...
		printf( __( 'К сожалению ваша покупка не удалась, попробуйте еще раз.<br /><a href="%1$s">Нажмите здесь что бы вернуться на страницу заказа</a>.', 'wpsc' ), get_option( "shopping_cart_url" ) );
	}
...
', 'wpsc' ), wpsc_currency_display( $total, array( 'display_as_html' => false ) ));
			...
			$total_price_html.= sprintf(__( 'Сумма: %s
...
", 'wpsc'), $purchase_log['id']);
			...
			if ( !empty($email) ) {
				...
				if ( ! $is_transaction ) {
					...
					$message =  __( '<H2>Ваш заказ принят</H2><p>Спасибо, что выбрали наш магазин <b>Veloman.by</b>, в течении часа с вами свяжется наш оператор, если этого не произойдет, пожалуйста наберите номер 8029 701-26-54 мтс или 8044 726-26-54 вел.</p>', 'wpsc' ) . "\n\r" . $payment_instructions ."Вы заказали:". $product_list  ."<h2>". $total_price_email; 
					$message_html  =  __('<H2>Ваш заказ принят</H2><p>Спасибо, что выбрали наш магазин <b>Veloman.by</b>, в течении часа с вами свяжется наш оператор, если этого не произойдет, пожалуйста наберите номер 8029 701-26-54 мтс или 8044 726-26-54 вел.</p>', 'wpsc' ) . "\n\r" . $payment_instructions ."<B>"."Вы заказали:"."</B>". $product_list_html  ."<h2>". $total_price_html; 
