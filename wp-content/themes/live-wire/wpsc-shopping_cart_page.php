...
if(wpsc_cart_item_count() < 1) :
   _e('Упс, Ваша корзина пуста.', 'wpsc') . "<a href=".get_option("product_list_url").">" . __('Посетите наш магазин', 'wpsc') . "</a>";
   return;
endif;
?>
<div id="checkout_page_container">
<h3><?php _e('Вы добавили следующий товар в корзину', 'wpsc'); ?></h3>
<table class="checkout_cart">
   <tr class="header">
      <th colspan="2" ><?php _e('Товар', 'wpsc'); ?></th>
      <th><?php _e('Кол-во', 'wpsc'); ?></th>
      <th><?php _e('Цена', 'wpsc'); ?></th>
      <th><?php _e('Сумма', 'wpsc'); ?></th>
        <th>&nbsp;</th>
   </tr>
   <?php while (wpsc_have_cart_items()) : wpsc_the_cart_item(); ?>
      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         <td style="width: 70px; height: 60px;" class="firstcol wpsc_product_image wpsc_product_image_<?php echo wpsc_the_cart_item_key(); ?>">
         <?php if('' != wpsc_cart_item_image()): ?>
			<?php do_action ( "wpsc_before_checkout_cart_item_image" ); ?>
            <img style="width: 60%; height: 60%;" src="<?php echo wpsc_cart_item_image(); ?>" alt="<?php echo wpsc_cart_item_name(); ?>" title="<?php echo wpsc_cart_item_name(); ?>" class="product_image" />
         ...
         <td style="text-align: left;" class="wpsc_product_name wpsc_product_name_<?php echo wpsc_the_cart_item_key(); ?>">
         ...
         <td class="wpsc_product_quantity wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform qty">
               <input type="text" style="text-align: center; width: 50px; margin-left: 25px;" name="quantity" size="1" value="<?php echo wpsc_cart_item_quantity(); ?>" />
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input type="submit" value="<?php _e('Обновить', 'wpsc'); ?>" name="submit" />
            </form>
         </td>

       
            <td style="text-align: center;" ><?php echo wpsc_cart_single_item_price(); ?></td>
         <td style="text-align: center;"  class="wpsc_product_price wpsc_product_price_<?php echo wpsc_the_cart_item_key(); ?>"><span class="pricedisplay"><?php echo wpsc_cart_item_price(); ?></span></td>

         <td style="text-align: center;"  class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               <input type="hidden" name="quantity" value="0" />
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <input type="submit" value="<?php _e('Удалить', 'wpsc'); ?>" name="submit" />
            </form>
         </td>
      </tr>
...
Вы можете заказать выбранный вами из каталога товар с 10-00 до 21-00 по следующим телефонам:

<ul>8 (044) 726-26-54 (Velcom)</ul>
<ul>8 (029) 701-26-54 (МТС)</ul>

<p>При заказе четко укажите модель велосипеда, размер рамы и цвет. 
Так же, если требуется доукомплектовать ваш велосипед дополнительными аксессуарами (фары, щитки, багажник и т.д.),
 укажите это при заказе, а наши специалисты подготовят, соберут, настроят и доставят вам ваш велосипед.</p>
<p>
Бесплатная доставка осуществляется от 1 800 000 бел.руб.
</p>
