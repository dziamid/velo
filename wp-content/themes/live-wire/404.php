...
<?php do_atomic( 'before_content' ); // live-wire_before_content ?>
	...
	<?php do_atomic( 'open_content' ); // live-wire_open_content ?>
		...
		<div class="hfeed">
			...
			<h1 class="error-404-title entry-title"><?php _e( 'Что случилось!?', 'live-wire' ); ?></h1>

				<div class="entry-content">
				
					<p>
						<?php printf( __( "Вы пытаетесь открыть страницу to %s, которая не существует. Попробуйте использовать поиск.", 'live-wire' ), '<code>' . home_url( esc_url( $_SERVER['REQUEST_URI'] ) ) . '</code>' ); ?>
					</p>
