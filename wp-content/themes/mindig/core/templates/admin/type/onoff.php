...
<div id="<?php echo $id ?>-container" <?php if( isset($deps) ): ?>data-field="<?php echo $id ?>" data-dep="<?php echo $deps['ids'] ?>" data-value="<?php echo $deps['values'] ?>" <?php endif ?>class="onoff_container yit_options rm_option rm_input rm_onoff">
    <div class="option">
        ...
        <input type="checkbox" name="<?php yit_field_name( $id ) ?>" id="<?php echo $id ?>" value="<?php echo yit_get_option( $id ) ?>" <?php checked( yit_get_option( $id ), 'yes' ); ?> class="on_off<?php if( yit_get_option( $id ) == 'yes' ): ?> onoffchecked<?php endif ?>" />
        <span>&nbsp;</span>
    </div>
